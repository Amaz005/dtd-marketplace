...
contract CALM721 is ERC721, ICALMNFT {
    ...
    function _hashDomain() internal pure returns(bytes32){
        return keccak256("EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)");

    }

    function _hashMintPermit() internal pure returns(bytes32) {
        return keccak256("MintPermit(uint256 tokenId,uint256 nonce,address currency,uint256 minimumPrice,address payee,uint256 kickoff,uint256 deadline,address recipient,bytes data)");
    }
